...
{
    [BurstCompile]
    internal unsafe struct CreateWireCapsuleJob : IJobParallelFor
    {
        ...
        public void Execute([AssumeRange(0, int.MaxValue)] int index)
        {
            ...
            float3 axis = math.normalizesafe(new float3(capsuleData->UpAxis.z, 0f, -capsuleData->UpAxis.x), front);
