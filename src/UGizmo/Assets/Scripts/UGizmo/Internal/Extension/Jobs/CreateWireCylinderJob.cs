...
{
    [BurstCompile]
    internal unsafe struct CreateWireCylinderJob : IJobParallelFor
    {
        ...
        public void Execute([AssumeRange(0, int.MaxValue)] int index)
        {
            ...
            float3 sphereOffset = capsuleData->UpAxis * (height - radius);
            float3 pos1 = capsuleData->Center + sphereOffset;
            float3 pos2 = capsuleData->Center - sphereOffset;

            WireCircleResult[index * 2 + 0] = new PrimitiveData(pos1, topRotation, new float3(radius), capsuleData->Color);
            WireCircleResult[index * 2 + 1] = new PrimitiveData(pos2, topRotation, new float3(radius), capsuleData->Color);
